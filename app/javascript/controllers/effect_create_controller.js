...
export default class extends Controller {
  ...
  nextStep(event) {
    ...
    const navItems = document.querySelectorAll('.M_SettingsNavigationEffect .A_TextTagFeed');
    let currentStep = 0;
    ...
    if (currentStep === 0) {
      if (this.hasEffectInfoTarget) this.effectInfoTarget.style.display = "none";
      if (this.hasInstructionBlockTarget) this.instructionBlockTarget.style.display = "block";
  
      navItems[0].classList.remove('A_TextTagFeedActive');
      navItems[1].classList.add('A_TextTagFeedActive');
    } else if (currentStep === 1) {
      if (this.hasInstructionBlockTarget) this.instructionBlockTarget.style.display = "none";
      if (this.hasPreviewAreaTarget) this.previewAreaTarget.style.display = "none";
      if (this.hasTagsLinkBlockTarget) this.tagsLinkBlockTarget.style.display = "block";
  
      navItems[1].classList.remove('A_TextTagFeedActive');
      navItems[2].classList.add('A_TextTagFeedActive');
    } else if (currentStep === 2) {
      if (this.hasTagsLinkBlockTarget) this.tagsLinkBlockTarget.style.display = "none";
      if (this.hasModerationBlockTarget) this.moderationBlockTarget.style.display = "block";
  ...
  getCurrentStep() {
    const activeItem = document.querySelector('.M_SettingsNavigationEffect .A_TextTagFeedActive');
