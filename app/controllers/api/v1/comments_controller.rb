class Api::V1::CommentsController < Api::V1::BaseController
  ...
  def destroy
    ...
    unless comment.user == current_user
      return render json: { error: "Вы можете удалять только свои комментарии" }, status: :forbidden
    end
    ...
    if comment.destroy
      render json: { 
        ...
        deleted_comment: comment_body 
      }, status: :ok
    else
      render json: { error: "Ошибка удаления комментария" }, status: :unprocessable_entity
    end
  end
